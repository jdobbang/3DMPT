...
from phalp.external.deep_sort_.tracker import Tracker, ByteTracker
...
class PHALP(nn.Module):
    ...
    def setup_deepsort(self):
        ...
        self.tracker = ByteTracker(self.cfg, metric, max_age=self.cfg.phalp.max_age_track, n_init=self.cfg.phalp.n_init, phalp_tracker=self, dims=[4096, 4096, 99])  
    ...
    def default_setup(self):
        ...
        try:
            video = str(self.cfg['video']['source']).split('/')[-1]
    ...
    def track(self):
        ...
        pkl_path = self.cfg.video.output_dir + self.cfg.track_dataset + "_" + str(self.cfg.video_seq) + '.pkl'
        ...
        try: 
            ...
            deblur_path = '/home/dobbang/jeon/PoseTrack/val_deblur/'+self.cfg.video_seq+'/'
            ...
            #for t_, frame_name in enumerate(list_of_frames):
                ...
                frame = str(frame_name).split('/')[-1]
                deblur_image_frame = cv2.imread(deblur_path+frame)
                ############ HMAR ##############
                detections = self.get_human_features(deblur_image_frame, pred_masks, pred_bbox, pred_scores, frame_name, pred_classes, t_, measurments, gt_tids, gt_annots, extra_data)
...
\
    ...
    def get_human_features(self, image, seg_mask, bbox, score, frame_name, cls_id, t_, measurments, gt=1, ann=None, extra_data=None):
        ...
        with torch.no_grad():
            ...
            uv_image_vis    = hmar_out['uv_image_vis']
            """
            for i in range(len(uv_image_vis)):
                uv = uv_image_vis[i]
                #storage_name = './uv_vis_/522/'+str(i)+'_'+str(frame_name).split('/')[-1]
                storage_name = '/home/dobbang/jeon/4dhumans/uv_vis_original/522/'+str(i)+'_'+str(frame_name).split('/')[-1]

                cv2.imwrite(storage_name,uv)
            """
