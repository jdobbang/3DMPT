...
from deep_sort_.tracker import Tracker,ByteTracker, KalmanBoxTracker
...
def test_tracker(opt, phalp_tracker: PHALP_tracker):
    ...
    tracker = ByteTracker(opt, metric, max_age=opt.max_age_track, n_init=opt.n_init, phalp_tracker=phalp_tracker, dims=[4096, 4096, 99])  
    ...
    try: 
        ...
        if 1:
            ...
            for t__ in range(d_):
                ...
                if 1: cv2.rectangle(rendered_, (0,0), (f_size[0], f_size[1]), (0,0,255), 4)
                cv2.imwrite(frame_key,rendered_)
                #if(t__-opt.n_init==0):
                if 1:
...
class options():
    def __init__(self):
        ...
        self.parser.add_argument('--track_dataset', type=str, default=None)
        self.parser.add_argument('--predict', type=str, default='APL')
        self.parser.add_argument('--storage_folder', type=str, default='storage')
        ...
        self.parser.add_argument('--render', type=str2bool, nargs='?', const=True, default=True)
...
if __name__ == '__main__':
    ...
    if (opt.track_dataset=='test_video'):
    ...
    if 1:
        ...
        #videos = np.sort(np.array(os.listdir('/home/dobbang/jeon/PoseTrack/val/')))
        #videos = np.array(['000809_mpii_test','024159_mpii_test'])
        videos = np.array(['024159_mpii_test'])

        for video in range(len(videos)):
            #if int(str(videos[video]).split('_')[0])>5833:
            
            opt.base_path       = '/home/dobbang/jeon/PoseTrack/val'
            opt.video_seq       = videos[video]
            opt.sample          =  ''
            opt.storage_foler = 'PHALP_0.8'
            test_tracker(opt, phalp_tracker)  
